...
console.log(showDropDown)
  ...
  return (
    <nav  className=" bg-black navbar text-white flex justify-between max-md:px-6 h-16 items-center max-md:background:radial-gradient(125%_125%_at_50%_10%,#00f_40%,#63t_100%) bg-gray-800	">
      <Link href={"/"} className="logo font-bold max-md:text-lg items-center flex"><span className="max-md:mx-6 mx-2n"><img src={teaLogo.src} width={55}/></span>GetMeaChai</Link>
...
<div >
<button onClick={()=>{if(showDropDown==false){setShowDropDown(true)}else{setShowDropDown(false)}}} id="dropdownInformationButton" data-dropdown-toggle="dropdownInformation" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800  " style={{overflow:"hidden"}} type="button">{userData?.username} <svg className="w-2.5 h-2.5 ms-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
...
<div onBlur={()=>setShowDropDown(false)}   id="dropdownInformation" style={{display:showDropDown?"block":"none"}} className="z-10 right-4 top-16 absolute hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
    ...
    <div className="py-2">
      <button  onClick={() => { router.push("/login"); signOut() ;}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Sign out</button>
    </div>
</div>
